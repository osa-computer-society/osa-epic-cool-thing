...
onready var waiting_for_response = false
onready var choice_box_open = false;
...
func _input(event): # Called on input
	...
	if (
		...
		elif !stop_advancement and !waiting_for_response:
		...
		elif waiting_for_response and !choice_box_open:
			emit_signal("turn_on_choices")
...
func change_profile(new_profile):
